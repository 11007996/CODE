...
namespace LxMail.Common
{
    public class MailKitHelper
    {
        ...
        public MailKitHelper(MailOptions _mailOptions)
        {
        ...
        private string SendMail(IEnumerable<MailboxAddress> toAddress, string subject, string text, string path = "", string html = "")
        {
            ...
            try
            {
                ...
                client.Connect(mailOptions.Smtp, mailOptions.Port, SecureSocketOptions.StartTls);
                //登录，发送
                //特别说明，对于服务器端的中文相应，Exception中有编码问题，显示乱码了
                client.Authenticate(System.Text.Encoding.Default, mailOptions.FromEmail, mailOptions.Password, CancellationToken.None);
