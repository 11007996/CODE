...
namespace CallSys
{
    public partial class FrmMasterError : Form
    {
        ...
        {
            Rectangle rec = new Rectangle(new Point(_owner.Location.X + _owner.Width, _owner.Location.Y), new Size(200, _owner.Height));
        ...
        {
            //查找未完成故障
            string sql = string.Format(@"SELECT TOP 100 Id,e.Area,e.Line,Dept,Machine,CallReason,MaxHandleTimes,MaxHelpTimes,StartTime, ComeTime,EndTime,
                                    e.HandlerNo,h1.HandlerName HandlerName,e.HelperNo,h2.HandlerName HelperName,Status,ErrorReason,FaultType,FaultContent,SolutionContent,ProdCount,SolverNo,SolverName
                                    FROM M_ErrorRecord_T e
                                    LEFT JOIN M_HandlerInfo_T h1 ON h1.HandlerNo=e.HandlerNo 
                                    LEFT JOIN M_HandlerInfo_T h2 ON h2.HandlerNo=e.HelperNo 
            ...
            if (dtErrors.Rows.Count > 0)
            {
                ...
                sql = string.Format(@"SELECT  e.ErrorId,e.HandlerNo,e.HandlerName,e.HandlerLevel,e.ErrorStatus,e.CreateTime,h.ImageUpdateTime
                                FROM M_ErrorHandleScan_T e LEFT JOIN M_HandlerInfo_T  h ON e.HandlerNo= h.HandlerNo
                                WHERE e.ErrorId in({0}) ORDER BY e.CreateTime ASC", string.Join(",", Ids));
