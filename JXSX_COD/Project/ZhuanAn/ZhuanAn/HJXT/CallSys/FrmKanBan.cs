...
namespace CallSys
{
    public partial class FrmKanBan : Form
    {
        ...
        private void FrmKanBan_Load(object sender, EventArgs e)
        {
            try
            {
                ss.Volume = BaseInfo.Volume;
                ss.Rate = BaseInfo.Rate;
                ss.SelectVoiceByHints(VoiceGender.Neutral);
               var a=  ss.GetInstalledVoices();
        ...
        /// 刷新工程师信息
        /// </summary>
        private void RefreshHandlerInfo()
        {
            //清除之前在gb_HanderInfo中自动生成的控件
            tlpHandlerInfo.Controls.Clear();
            //获取新的人员信息
            DataTable handlersInfo = SelectCurrWeekHandlerData();
            if (handlersInfo == null || handlersInfo.Rows.Count <= 0)
            {
                tlpHandlerInfo.Visible = false;
                return;
            }
            //挂载人员信息组件
            tlpHandlerInfo.Visible = true;
            tlpHandlerInfo.ForeColor = Color.Red;
            for (int i = 0; i < handlersInfo.Rows.Count; i++)
            {
                int colIndex = i % 4;
                int rowIndex = Int32.Parse(Math.Floor((double)(i / 4)).ToString());
                MountHandlerInfoComponent(i, handlersInfo, rowIndex, colIndex);
            }
            tlpHandlerInfo.Refresh();
        }

        /// <summary>
        /// 查出本周所有工程师（非管理员）的基础信息、本周维护总数，本周未解决数，本周维护时长
        /// </summary>
        private DataTable SelectCurrWeekHandlerData()
        {
            //【HandlerNo工号、HandlerName姓名，HandlerState状态，HandleQty本周维护总计，UntreatedQty本周未解决总计，TotalTimes本周维护时长】
            string sql = string.Format(@"SELECT HandlerNo,HandlerName,HandlerState,ImageUpdateTime,'0' AS HandleQty,'0' AS UntreatedQty,'0' TotalTimes
                    FROM m_HandlerInfo_t WHERE Area like '%{0}%' AND HandlerDept like N'%{1}%' AND HandlerLevel ='1' AND UseFlag='Y'
                    ORDER BY HandlerState DESC;", Area, BaseInfo.Dept);
            ...
            sql += string.Format(@"SELECT HandlerNo,SUM(DATEDIFF(SECOND,Cometime,Endtime))as TotalTimes
                    FROM  M_ErrorRecord_T  WHERE Area='{0}' AND  Endtime is not null  AND Cometime is not null
				    AND Starttime >=(SELECT CONVERT(datetime, FLOOR(CONVERT(float,GETDATE())/7)*7) AS StartOfWeek)
				    AND Starttime <= (select CONVERT(datetime, FLOOR(CONVERT(float,GETDATE())/7)*7)+6 AS EndOfWeek)
                    GROUP BY HandlerNo;", Area);

            DataSet dsHandlerInfo = DBUtil.GetDataSet(sql, new string[] { "dt_HandlerInfo", "dt_HandleQty", "dt_UntreatedQty", "dt_TotalTimes" });
            if (dsHandlerInfo == null) return null;
            DataTable dt_HandlerInfo = dsHandlerInfo.Tables["dt_HandlerInfo"];
            DataTable dt_HandleQty = dsHandlerInfo.Tables["dt_HandleQty"];
            DataTable dt_UntreatedQty = dsHandlerInfo.Tables["dt_UntreatedQty"];
            DataTable dt_TotalTimes = dsHandlerInfo.Tables["dt_TotalTimes"];

            //遍历每个工程师，并将 【HandleQty本周维护总计，UntreatedQty本周未解决总计，TotalTimes本周维护时长】插入工程师信息表中
            foreach (DataRow handler in dt_HandlerInfo.Rows)
            {
                string handlerNo = handler["HandlerNo"].ToString();
                string filterWhere = string.Format("HandlerNo = '{0}'", handlerNo);
                if (dt_HandleQty.Rows.Count > 0)
                {
                    DataRow[] rows = dt_HandleQty.Select(filterWhere);
                    if (rows != null && rows.Length > 0 && rows[0]["HandleQty"] != null)
                    {
                        handler["HandleQty"] = rows[0]["HandleQty"];
                    }
                }
                if (dt_UntreatedQty.Rows.Count > 0)
                {
                    DataRow[] rows = dt_UntreatedQty.Select(filterWhere);
                    if (rows != null && rows.Length > 0 && rows[0]["UntreatedQty"] != null)
                    {
                        handler["UntreatedQty"] = rows[0]["UntreatedQty"];
                    }
                }
                if (dt_TotalTimes.Rows.Count > 0)
                {
                    DataRow[] rows = dt_TotalTimes.Select(filterWhere);
                    if (rows != null && rows.Length > 0 && rows[0]["TotalTimes"] != null)
                    {
                        handler["TotalTimes"] = rows[0]["TotalTimes"];
                    }
                }
            }
            return dt_HandlerInfo;
        }
        ...
        private void MountHandlerInfoComponent(int i, DataTable dt, int rowIndex, int colIndex)
        {
            ...
            panel.BackColor = System.Drawing.Color.FromArgb(((int)(((byte)(255)))), ((int)(((byte)(155)))), ((int)(((byte)(51)))));
            panel.Dock = System.Windows.Forms.DockStyle.Fill;
            panel.Location = new System.Drawing.Point(0, 0);
            panel.Margin = new System.Windows.Forms.Padding(0, 0, (int)(2 * xScale), (int)(2 * xScale));
            panel.Name = "panelHandlerInfo" + i;
            panel.Size = new System.Drawing.Size(79, 128);
            panel.TabIndex = 0;
            // 
            // pbHandlePic
            // 
            PictureBox pbHandlerPic = new PictureBox();
            pbHandlerPic.Dock = System.Windows.Forms.DockStyle.Top;
            pbHandlerPic.ErrorImage = global::CallSys.Properties.Resources.defualt_face;
            pbHandlerPic.Image = global::CallSys.Properties.Resources.defualt_face;
            pbHandlerPic.Location = new System.Drawing.Point(0, 0);
            pbHandlerPic.Margin = new System.Windows.Forms.Padding(0);
            pbHandlerPic.Name = "pbHandlerPic" + i;
            pbHandlerPic.Size = new System.Drawing.Size(79, (int)(84 * yScale));
            pbHandlerPic.SizeMode = System.Windows.Forms.PictureBoxSizeMode.StretchImage;
            pbHandlerPic.TabIndex = 0;
            pbHandlerPic.TabStop = false;
            //获取照片路径
            try
            {
                if (dt.Rows[i]["ImageUpdateTime"] != DBNull.Value)
                {
                    pbHandlerPic.Image = FileUtil.GetCacheFile(dt.Rows[i]["HandlerNo"].ToString(), Convert.ToDateTime(dt.Rows[i]["ImageUpdateTime"]));
                }
                else
                {
                    pbHandlerPic.Image = Resources.defualt_face;
                }
            }
            catch (Exception)
            {
            ...
            // labHandlerNo
            // 
            Label labHandlerNo = new Label();
            labHandlerNo.AutoSize = true;
            labHandlerNo.Font = new System.Drawing.Font("宋体", (float)(6F * xScale), System.Drawing.FontStyle.Bold);
            labHandlerNo.ForeColor = Color.White;
            labHandlerNo.Location = new System.Drawing.Point(0, (int)(85 * yScale));
            labHandlerNo.Name = "labHandlerNo" + i;
            labHandlerNo.Size = new System.Drawing.Size(48, 15);
            labHandlerNo.TabIndex = 1;
            labHandlerNo.Enabled = true;
            string handlerNo = "工号:" + dt.Rows[i]["HandlerNo"].ToString();
            labHandlerNo.Text = handlerNo;
            // 
            // labHandleQty
            // 
            ...
            labHandleQty.Location = new System.Drawing.Point(0, (int)(95 * yScale));
            ...
            labUntreatedQty.Location = new System.Drawing.Point(0, (int)(105 * yScale));
            ...
            labTotalTimes.Location = new System.Drawing.Point(0, (int)(115 * yScale));
            ...
            panel.Controls.Add(pbHandlerPic);
            panel.Controls.Add(labHandlerNo);
            panel.Controls.Add(labHandleQty);
            panel.Controls.Add(labUntreatedQty);
            panel.Controls.Add(labTotalTimes);

            tlpHandlerInfo.Controls.Add(panel, colIndex, rowIndex);
        }
        ...
        private void RefreshErrorInfo()
        {
            try
            {
                ...
                string sql = string.Format(@"SELECT  TOP 100 '' FormatColumn,e.Line+e.Machine MachineNo,e.Line,e.Machine,e.Dept,e.StartTime,e.ComeTime,'' WaitedTime,
                                        h1.HandlerName HandlerName,h2.HandlerName HelperName,e.Status,e.CallReason,'' HandleTime,e.EndTime,e.Id
                                        ...
                                        WHERE e.Area='{0}'  AND Status<>'Y' AND Status<>'N' 
                                        ORDER BY Status ASC,StartTime DESC", Area);
        ...
        private void SendVoiceBroadcast(DataTable dt)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(machine) && !string.IsNullOrEmpty(state))
                    {
                        ...
                        if (!NoHandleSpeechCount.ContainsKey(id))
                        {
                            ...
                            ss.SpeakAsync(string.Format("机台故障,请{0}人员到{1}{2}机台处理", dept, line, machine));
                            ss.SpeakAsync(string.Format("机台故障,请{0}人员到{1}{2}机台处理", dept, line, machine));
                        }
                        else if ("A".Equals(state))
                        {//未接单，每分钟再播一次

                            if (NoHandleSpeechCount[id] % 6 == 0)
                            {
                                ss.SpeakAsync(string.Format("机台故障,请{0}人员到{1}{2}机台处理", dept, line, machine));
                                ss.SpeakAsync(string.Format("机台故障,请{0}人员到{1}{2}机台处理", dept, line, machine));
                            }
                        ...
                        {//呼叫支援
                            ...
                            if (NeedHelpSpeechCount[id] % 6 == 0)
                            {
                                ss.SpeakAsync(line + machine + "机台故障，请高级工程师尽快到现场处理");
                            }
                ...
                //List<int> intesectNewErrors = NewErrorList.Intersect(tempErrors).ToList();
                //List<int> intesectOvertimeErrors = OvertimeErrorList.Intersect(tempErrors).ToList();
                //NewErrorList = intesectNewErrors;
                //OvertimeErrorList = intesectOvertimeErrors;
        ...
        private DataTable SelectMachineErrorToChartData()
        {
            ...
            //故障机台
            string sql = string.Format(@"SELECT TOP {0} Line+Machine MachineNo  FROM M_ErrorRecord_T WHERE Area='{1}' AND StartTime>='{2}' 
                                        GROUP BY Line,Machine  ORDER BY Count(Line+Machine) DESC", MachineNum, Area, currWeekStart);
            ...
            if (machines.Count < MachineNum)
            {
                ...
                sql = string.Format(@"SELECT TOP {0} Line+Machine MachineNo FROM M_ErrorRecord_T WHERE Area=N'{1}'  And StartTime>='{2}' AND StartTime<'{3}' 
                                    AND Line+Machine NOT IN('{4}') GROUP BY Line,Machine ORDER BY Count(*) DESC ", MachineNum - machines.Count, Area, preWeekStart, currWeekStart, string.Join("','", machines));
        ...
        private DataTable SelectMachineErrorCountWithWeek(int preWeek, IList<string> machineNo)
        {
            ...
            string sql = string.Format("SELECT Line+Machine AS MachineNo,Count(*) ErrorCount FROM M_ErrorRecord_T WHERE Area='{0}' AND StartTime>='{1}' AND StartTime<'{2}' AND Line+Machine in('{3}')   GROUP BY Line,Machine",
                                        Area, start, end, string.Join("','", machineNo));
        ...
        private void FrmKanBan_Resize(object sender, EventArgs e)
        {
            ...
            SetControl(xScale, yScale, tlpHandlerInfo);
