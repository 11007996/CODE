...
namespace Basic
{
    public partial class FrmLineInfo : Form
    {
        ...
        {
            dgvLineInfo.DataSource = DBUtil.GetDataTable("SELECT * FROM S_LineInfo_T Order By Id");
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = string.Format("SELECT 1 FROM S_LineInfo_T WHERE   Line=N'{0}'", tbLine.Text.Trim());
                DataTable dt = DBUtil.GetDataTable(sql);
                if (dt != null && dt.Rows.Count > 0)
                {
                    labMessage.Text = "已存在相同产线名称";
                    return;
                }
                //新增
                sql = string.Format("INSERT INTO S_LineInfo_T(Factory,Area,Line) VALUES(N'{0}',N'{1}',N'{2}')", BaseInfo.Factory, tbArea.Text.Trim(), tbLine.Text.Trim());
                DBUtil.ExecSQL(sql);
                RefreshLineInfo();
        ...
        //删除
        private void dgvLineInfo_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            e.Cancel = true;
            labMessage.Text = "";
            ...
            string id = e.Row.Cells["dgcId"].Value.ToString();
            string area = e.Row.Cells["dgcArea"].Value.ToString();
            ...
            {
                string sqlEdit = string.Format("DELETE S_LineInfo_T  WHERE Id='{0}'", id);
