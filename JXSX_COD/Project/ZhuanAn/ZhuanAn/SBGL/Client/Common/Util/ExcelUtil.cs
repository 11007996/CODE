                ...
                for (int i = 0; i <= sheet.LastRowNum; i++)  //LastRowNum 是当前表的总行数-1（注意）
                {
                    ...
                    if (row != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //读取该行的第j列数据 CellType = Numeric
                                if (row.GetCell(j) == null)
                                {
                                    dr[j] = null;
                                }
                                else if (row.GetCell(j).CellType == CellType.Numeric && HSSFDateUtil.IsCellDateFormatted(row.GetCell(j)))
                                {//日期数据处理
                                    dr[j] = row.GetCell(j).DateCellValue.ToString("yyyy-MM-dd HH:mm:ss");
                                }
                                else
                                {
