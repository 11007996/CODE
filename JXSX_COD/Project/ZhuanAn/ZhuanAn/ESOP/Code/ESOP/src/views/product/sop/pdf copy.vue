...
export default {
    ...
    methods: {
        ...
        initPdfWidth() {
            let pdfWidthList = this.pdfSizeList.map((item) => {
                return item.width;
            });
            let pdfHeightList = this.pdfSizeList.map((item) => {
                return item.height;
            });
            let pdf_width = pdfWidthList[this.currentPage - 1];
            let pdf_height = pdfHeightList[this.currentPage - 1];
            this.$nextTick(() => {
                let sc = document.querySelector(".pdfArea").clientHeight / document.querySelector(".pdfArea").clientHeight
