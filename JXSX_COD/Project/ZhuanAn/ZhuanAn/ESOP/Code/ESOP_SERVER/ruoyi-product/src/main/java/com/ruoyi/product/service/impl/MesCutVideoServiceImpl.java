...
import com.ruoyi.product.domain.CmdResult;
...
@Service
public class MesCutVideoServiceImpl implements IMesCutVideoService {

    //分割视频的大小，装包动作为了避免越界。long应该够使了。。。
//    private long blockSize = 1 * new Long(1024) * 1024 * 1024;
    private long blockSize = 15 * 1024 * 1024; //15MB
...
//        commands.add("ffmpeg");
        commands.add("D:\\app\\FFmpeg\\ffmpeg-6.0-full_build\\bin\\ffmpeg.exe");
        commands.add("-i");
        ...
        if (result.isSuccess()) {
            ...
            if (time.isEmpty()) {
                throw new IllegalStateException("视频时长格式不正确");
            }
            return time;
        } else {
            return "";
...
//        if (!filePath.endsWith(".mp4")||!filePath.endsWith(".mts")) {
