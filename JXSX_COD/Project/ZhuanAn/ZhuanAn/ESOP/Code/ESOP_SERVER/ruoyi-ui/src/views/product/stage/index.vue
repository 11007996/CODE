...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['product:stage:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:stage:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="stageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="区段编号" align="center" prop="stageId" />
      <el-table-column label="区段名称" align="center" prop="stageName" />
      <el-table-column label="状态" align="center" prop="status" >
        <template slot-scope="scop">
          <el-switch
          v-model="scop.row.status"
          active-value="0"
          inactive-value="1"
          @change="handleStatusChange(scop.row)"
          ></el-switch>
        </template>
      </el-table-column>
...
<!--          >删除</el-button>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="区段名称" prop="stageName">
          <el-input v-model="form.stageName" placeholder="请输入区段名称" />
        </el-form-item>
...
<script>
import { listStage, getStage, delStage, addStage, updateStage, changeStageStatus } from "@/api/product/stage";
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.stageName + '"区段吗？').then(function() {
        return changeStageStatus(row.stageId, row.status);
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除区段管理编号为"' + stageIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/stage/export', {
        ...this.queryParams
