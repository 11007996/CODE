...
namespace EAM.Admin.WebApi.Controllers.System
{
    ...
    public class SysUserController : BaseController
    {
        ...
        public IActionResult GetInfo(int userId)
        {
            ...
            dic.Add("roles", RoleService.SelectRoleAll());
            //dic.Add("roles", SysUser.IsAdmin(userId) ? roles : roles.FindAll(f => !f.IsAdmin()));
            dic.Add("factorys", FactoryService.GetAll());
            ...
            if (userId > 0)
            {
                ...
                dic.Add("factoryIds", sysUser.FactoryIds);
            }
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        [HttpPost("syncUserToEmp")]
        [Log(Title = "用户同步到员工", BusinessType = BusinessType.UPDATE)]
        [ActionPermissionFilter(RolePermi = "admin")]
        public IActionResult SyncUserToEmp([FromBody] SysUserToEmployeeDto parm)
        {
            return SUCCESS(UserService.SyncSysUserToEmployee(parm));
        }
    }
}