...
namespace EAM.Admin.WebApi.Controllers.System
{
    ...
    public class SysDeptController : BaseController
    {
        ...
        /// <param name="dept"></param>
        /// <returns></returns>
        [HttpGet("factoryTreeselect")]
        public IActionResult TreeSelectByFactory(SysDeptQueryDto dept)
        {
            string factoryId = HttpContext.GetFactoryId();
            dept.FactoryId = factoryId;
            var depts = DeptService.GetSysDeptByFactorys(dept);
        ...
        {
            if (DeptService.Queryable().Count(it => it.ParentId == deptId && it.DelFlag == (int)DeleteFlagEnum.存在) > 0)
            {
                return ToResponse(ResultCode.CUSTOM_ERROR, $"存在下级部门，不允许删除");
            }
            if (UserService.Queryable().Count(it => it.DeptId == deptId && it.DelFlag == (int)DeleteFlagEnum.存在) > 0)
            {
