...
namespace EAM.Admin.WebApi.Controllers.System
{
    ...
    public class SysUserRoleController : BaseController
    {
        ...
        /// <param name="roleUserQueryDto"></param>
        /// <returns></returns>
        [HttpGet("GetExcludeFactoryUsers")]
        public IActionResult GetExcludedFactoryUsers([FromQuery] RoleUserQueryDto roleUserQueryDto)
        {
            if (roleUserQueryDto.RoleId <= 0)
            {
                throw new CustomException(ResultCode.PARAM_ERROR, "roleId不能为空");
            ...
            // 获取未添加用户
            var list = SysUserRoleService.GetExcludedFactoryUsersByRoleId(roleUserQueryDto);

            return SUCCESS(list, TIME_FORMAT_FULL);
        }

        /// <summary>
        /// 根据角色编号获取已分配的用户(给工厂使用)
        /// </summary>
        /// <param name="roleUserQueryDto"></param>
        /// <returns></returns>
        [HttpGet("factory/list")]
        [ActionPermissionFilter(Permission = "system:rolefactory:user:list")]
        public IActionResult GetFactoryRoleUserList([FromQuery] RoleUserQueryDto roleUserQueryDto)
        {
            var list = SysUserRoleService.GetSysUsersByRoleId(roleUserQueryDto);
