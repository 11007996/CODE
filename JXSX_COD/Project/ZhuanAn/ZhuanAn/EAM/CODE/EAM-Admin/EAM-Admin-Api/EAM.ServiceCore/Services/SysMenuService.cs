...
using EAM.Model.System.Enums;
using EAM.Model.System.Generate;
    ...
    public class SysMenuService : BaseService<SysMenu>, ISysMenuService
    {
        ...
        public List<string> SelectMenuPermsByUserId(long userId)
        {
            var menus = Context.Queryable<SysMenu, SysRoleMenu, SysUserRole, SysRole>((m, rm, ur, r) => new JoinQueryInfos(
                ...
                .WithCache(60 * 10) //优先从缓存中查询，缓存10分钟
        ...
        /// </summary>
        /// <returns></returns>
        public List<SysMenu> SelectDataboardMenuByUser(long userId)
        {
            var parentMenu = Queryable().Where(it => it.Status == ((int)MenuStatus.正常).ToString() && it.MenuType == MenuType.M.ToString() && it.Path == "databoard").First();
            if (parentMenu == null)
                return null;
            ...
            if (!SysRoleService.IsAdmin(userId))
            {
                menuList = Context.Queryable<SysMenu, SysRoleMenu, SysUserRole, SysRole>((m, rm, ur, r) => new JoinQueryInfos(
                   JoinType.Left, m.MenuId == rm.Menu_id,
                   JoinType.Left, rm.Role_id == ur.RoleId,
                   JoinType.Left, ur.RoleId == r.RoleId
                ))
                ...
                .ToChildList(it => it.ParentId, parentMenu.MenuId);
            }
            else
            {
                menuList = Queryable()
            ...
            menuList = menuList.Where(it => it.MenuType == MenuType.C.ToString()).ToList();
