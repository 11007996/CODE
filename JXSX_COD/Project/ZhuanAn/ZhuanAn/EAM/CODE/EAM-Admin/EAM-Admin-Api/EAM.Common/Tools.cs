...
namespace EAM.Common
{
    public class Tools
    {
        ...
        public static string SerializeHttpForm(object query, string prefix = "?")
        {
            if (query is null)
            {
                return null;
            ...
            var properties = from property in query.GetType().GetProperties()
                             ...
                             select memberName + "=" + System.Web.HttpUtility.UrlEncode(value.ToString());
            var queryString = string.Join("&", properties);
            return string.IsNullOrWhiteSpace(queryString) ? "" : prefix + queryString;
        }

        public static string SerializeHttpForm(object query)
        {
            if (query is null)
            {
                return null;
        ...
        /// <summary>
        /// 判断字符串是否为有效的空格分隔的十六进制字节序列（如 "00-FF"）。
        /// 要求：
        /// - 每个字节由两个十六进制字符组成（00~FF）
        /// - 字节之间用单个"-"分隔
        /// </summary>
        public static bool IsValidHexByteString(string input)
        {
            ...
            // 正则：匹配一个或多个 "XX"（两个十六进制字符），用“-”连接
            return Regex.IsMatch(input, @"^[0-9A-Fa-f]{2}(-[0-9A-Fa-f]{2})*$");
        }
    }
}