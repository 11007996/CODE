...
namespace EAM.Service.Consumable
{
    ...
    /// 耗品储位信息Service业务层处理
    /// </summary>
    [AppService(ServiceType = typeof(IConsumableStorageSpaceService), ServiceLifetime = LifeTime.Transient)]
    public class ConsumableStorageSpaceService : BaseService<ConsumableStorageSpace>, IConsumableStorageSpaceService
    {
        public ConsumableStorageSpaceService(IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
        }
        ...
        /// <returns></returns>
        public PagedInfo<ConsumableStorageSpaceDto> GetList(ConsumableStorageSpaceQueryDto parm)
        {
            ...
            var response = Queryable()
                ...
                .ToPage<ConsumableStorageSpace, ConsumableStorageSpaceDto>(parm);
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public List<ConsumableStorageSpace> GetTreeList(ConsumableStorageSpaceTreeQueryDto parm)
        {
            List<ConsumableStorageSpace> response = new List<ConsumableStorageSpace>();

            if (!string.IsNullOrEmpty(parm.StorageName))
            {//条件
                List<ConsumableStorageSpace> sp = Queryable().Where(it => it.StorageName.Contains(parm.StorageName)).ToList();
        ...
        /// <returns></returns>
        public ConsumableStorageSpace GetInfo(int StorageId)
        {
            var response = Queryable()
        ...
        /// <returns></returns>
        public ConsumableStorageSpace AddStorageSpace(ConsumableStorageSpace model)
        {
            ...
            ConsumableStorageSpace parentSS = GetFirst(it => it.StorageId == model.ParentId);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int UpdateStorageSpace(ConsumableStorageSpace model)
        {
            ...
            // 更新所有子集的 【祖级信息】、【储位全名】
            ConsumableStorageSpace newParentSS = GetFirst(it => it.StorageId == model.ParentId);
            ConsumableStorageSpace oldSS = GetFirst(it => it.StorageId == model.StorageId);
            if (newParentSS != null)
            {// 新的有上级
        ...
        {
            //判断是否存在储位信息
            ConsumableStorageSpace model = GetInfo(storageId);
            if (model == null)
            ...
            //判断是否有耗品与当前储位关联
            count = Context.Queryable<ConsumableStorage>().Where(it => it.StorageId == storageId).Count();
            if (count > 0)
                throw new CustomException($"当前储位有耗品库存信息，请先删除当前储位关联下的耗品库存信息");
            return Delete(model);
        }
        ...
        {
            // 获取有关联的所有子储位
            List<ConsumableStorageSpace> all = Queryable().ToList();
            List<ConsumableStorageSpace> children = GetChildrenStorageSpace(all, storageSpaceId);
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        private static Expressionable<ConsumableStorageSpace> QueryExp(ConsumableStorageSpaceQueryDto parm)
        {
            var predicate = Expressionable.Create<ConsumableStorageSpace>();
        ...
        private void UpdateParentStorageStatusNormal(ConsumableStorageSpace storageSpace)
        {
        ...
        /// <returns></returns>
        private List<ConsumableStorageSpace> GetChildrenStorageSpace(List<ConsumableStorageSpace> storageSpaces, int storageId)
        {
            return storageSpaces.FindAll(delegate (ConsumableStorageSpace item)
            {
