...
namespace EAM.Common.Wechat
{
    public class WxNoticeHelper
    {
        ...
        public static (int, string) SendMsg(string title, string content, string toUser = "", MsgType msgType = MsgType.text)
        {
            ...
            {
                return (0, "未设置接收对象");//"@all" 发送给所有用户
            }
            if (string.IsNullOrEmpty(title))
            {
                return (0, "title不能为空");
            }
            if (string.IsNullOrEmpty(WxCorpConfig.CORPID))
            {
        ...
        {
            string getTokenUrl = $"{GetTokenUrl}?corpid={WxCorpConfig.CORPID}&corpsecret={WxCorpConfig.CORPSECRET}";
