...
namespace EAM.Repository
{
    ...
    public static class QueryableExtension
    {
        ...
        /// <param name="parm">分页参数</param>
        /// <returns></returns>
        public static PagedInfo<T> ToPageNoSort<T>(this ISugarQueryable<T> source, PagerInfo parm)
        {
            var page = new PagedInfo<T>();
            var total = 0;
            page.PageSize = parm.PageSize;
    ...
    public class BaseRepository<T> : SimpleClient<T> where T : class, new()
    {
        ...
        public BaseRepository(IHttpContextAccessor context) : base()
        {
            //根据厂区获取
            if (context.HttpContext != null)
            {
                //通过厂区获取对应型业务数据库。
                var configId = HttpContextExtension.GetFactoryId(context.HttpContext);
                Context = DbScoped.SugarScope.GetConnectionScope(configId);//根据类传入的ConfigId自动选择
            }
            else
            {
                Context = DbScoped.SugarScope.GetConnectionScope(0);//没有默认db0
            }
            //Context = DbScoped.SugarScope.GetConnectionScopeWithAttr<T>();
            itenant = DbScoped.SugarScope;//设置租户接口
        }
        ...
        //第2版：其中任意一个不需要的库连接有问题，就会出现异常，提示连接有问题，虽然你可能没使用到。
        //public DbResult<bool> UseTran(Action action)
        //{
        //    try
        //    {
        //        DbResult<bool> r = new();
        //        //不要使用DbScoped.SugarScope.CreateContext创建事务，会因为其中任意一个库的连接有问题，导致所有正常的其他库事务出现问题
        //        using (var tran = DbScoped.SugarScope.CreateContext(Context.Ado.IsNoTran()))
        //        {
        //            action();
        //            tran.Commit();
        //            r.IsSuccess = true;
