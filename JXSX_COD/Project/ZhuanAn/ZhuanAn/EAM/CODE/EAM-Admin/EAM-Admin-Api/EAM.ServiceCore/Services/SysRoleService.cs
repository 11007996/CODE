...
namespace EAM.ServiceCore.Services
{
    ...
    public class SysRoleService : BaseService<SysRole>, ISysRoleService
    {
        ...
        /// 根据条件分页查询角色数据
        /// </summary>
        /// <param name="parm">角色信息</param>
        /// <returns>角色数据集合信息</returns>
        public PagedInfo<SysRole> SelectRoleList(SysRoleQueryDto parm)
        {
            var exp = Expressionable.Create<SysRole>();
            exp.And(role => role.DelFlag == 0);
            exp.AndIF(!string.IsNullOrEmpty(parm.RoleName), role => role.RoleName.Contains(parm.RoleName));
            exp.AndIF(parm.Status != -1, role => role.Status == parm.Status);
            exp.AndIF(!string.IsNullOrEmpty(parm.RoleKey), role => role.RoleKey == parm.RoleKey);
            exp.AndIF(!string.IsNullOrEmpty(parm.FactoryId), role => role.FactoryId == parm.FactoryId);
        ...
        public int UpdateSysRole(SysRole sysRole)
        {
            ...
            .SetColumns(it => it.FactoryId == sysRole.FactoryId)
        ...
        /// <returns></returns>
        public PagedInfo<DictDataDto> DictRole(SysRoleQueryDto parm)
        {
            return Queryable()
                .Where(it => it.Status == 0)
                .WhereIF(!string.IsNullOrEmpty(parm.RoleName), it => it.RoleName.Contains(parm.RoleName))
                .WhereIF(!string.IsNullOrEmpty(parm.RoleKey), it => it.RoleKey.Contains(parm.RoleKey))
                .Select(it => new DictDataDto()
                {
                    DictValue = it.RoleId.ToString(),
                    DictLabel = it.RoleName,
                    DictDesc = it.RoleKey
                }).ToPage(parm);
        }
    }
