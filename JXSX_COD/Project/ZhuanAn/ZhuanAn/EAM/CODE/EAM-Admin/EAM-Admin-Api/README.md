<h2 align="center"> EAM.Admin.NET后台管理系统</h2>
<h4 align="center">基于.Net7 + vue3.x前后端分离的.net快速开发框架</h4>
...
Vue 版前端技术栈 ：基于 vue3.x、vuex、vue-router 、vue-cli 、axios、 element-ui、echats、i18n 国际化等，前端采用 vscode 工具开发
...
```
├─EAM.Service                ->[你的业务服务层类库]：提供WebApi接口调用；
├─EAM.ServiceCore            ->[系统服务层类库]：提供WebApi接口调用；
├─EAM.Repository             ->[仓库层类库]：方便提供有执行存储过程的操作；
├─EAM.Model                	->[实体层类库]：提供项目中的数据库表、数据传输对象；
├─EAM.Admin.WebApi           ->[webapi接口]：为Vue版或其他三方系统提供接口服务。
├─EAM.Tasks               		->[定时任务类库]：提供项目定时任务实现功能；
├─EAM.CodeGenerator          ->[代码生成功能]：包含代码生成的模板、方法、代码生成的下载。
├─document               		->[文档]：数据库脚本
```
