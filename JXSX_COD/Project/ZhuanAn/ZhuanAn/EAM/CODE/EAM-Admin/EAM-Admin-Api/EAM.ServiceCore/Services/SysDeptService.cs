...
namespace EAM.ServiceCore.Services
{
    ...
    public class SysDeptService : BaseService<SysDept>, ISysDeptService
    {
        ...
        /// <param name="dept"></param>
        /// <returns></returns>
        public List<SysDept> GetSysDeptByFactorys(SysDeptQueryDto dept)
        {
            string rootDeptId = null;
            if (!string.IsNullOrEmpty(dept.FactoryId))
            {//获取厂区对应的根部门
                SysFactory fa = Context.Queryable<SysFactory>().Where(it => it.FactoryId == dept.FactoryId).First();
                if (fa.RootDeptId == null || fa.RootDeptId == 0)
                    return new List<SysDept>();
                else
                    rootDeptId = fa.RootDeptId.Value.ToString();
            }
            ...
            if (!string.IsNullOrEmpty(rootDeptId) && response != null && response.Count > 0)
            {//去掉不在根部门下的部门
                for (int i = response.Count - 1; i >= 0; i--)
                {
                    if (response[i].DeptId.ToString() == rootDeptId)
                        continue;
                    string Ancestors = response[i].Ancestors?.ToString();
                    if (string.IsNullOrEmpty(Ancestors) || !Ancestors.Split(',').Contains(rootDeptId))
                        response.RemoveAt(i);
                }
            }
        ...
        public List<TreeSelectVo> BuildDeptTreeSelect(List<SysDept> depts)
        {
            ...
            List<TreeSelectVo> treeMenuVos = new();
