<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button plain type="success" icon="plus" @click="handleDeptExpand">扩展配置</el-button>
      </el-col>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="230">
        <template #default="scope">
          <el-button text icon="edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:dept:update']">
            {{ $t('btn.edit') }}
          </el-button>
          <el-button text icon="plus" @click="handleAdd(scope.row)" v-hasPermi="['system:dept:add']">
            {{ $t('btn.add') }}
          </el-button>
          <el-button text v-if="scope.row.parentId != 0" icon="delete" @click="handleDelete(scope.row)" v-hasPermi="['system:dept:remove']">
            {{ $t('btn.delete') }}
          </el-button>
        </template>
      </el-table-column>
...
/** 扩展操作 */
function handleDeptExpand() {
  var hasPermi = proxy.$auth.hasPermi('system:dept:expand:list')
  if (hasPermi) {
    router.push({ path: '/system/dept/SysDeptExpand' })
  } else {
