<template>
  <DataSelectorDialog
    ...
    <template #tableColumns>
      <!-- <el-table-column prop="fixtureId" label="ID" width="80" /> -->
      <el-table-column prop="fixtureName" label="治具名称" />
      <el-table-column prop="series" label="系列" />
      <el-table-column prop="price" label="单价" />
      <el-table-column prop="defaultQty" label="配置数量" />
      <el-table-column prop="totalIdleQty" label="闲置数量" />
    </template>
  </DataSelectorDialog>
</template>

<script setup="fixtureselectorforidle">
import { ref } from 'vue'
import DataSelectorDialog from '@/components/DataSelectorDialog/index.vue'
import { idleFixtureBase } from '@/api/fixture/fixtureBase.js'
...
const loadData = async (params) => {
  const res = await idleFixtureBase(params)
  return res.data
}
</script>
