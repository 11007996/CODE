    ...
    <el-table
      ...
      <el-table-column prop="isSend" label="是否发送" align="center">
        <template #default="{ row }">
          <DictTag :options="options.isSend_options" :value="row.isSend"></DictTag>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="220">
        <template #default="scope">
          <el-button type="primary" icon="view" text @click="handlePreview(scope.row)"> 预览 </el-button>
          <el-button type="danger" icon="delete" text v-hasPermi="['emaillog:delete']" @click="handleDelete(scope.row)"> 删除 </el-button>
          <el-button type="primary" icon="upload" text v-hasPermi="['tool:email:send']" @click="handleSend(scope.row)" v-if="scope.row.isSend == 0">
            发送
          </el-button>
        </template>
      </el-table-column>
