    ...
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="发送类型" prop="sendType">
        <el-select clearable v-model="queryParams.sendType" placeholder="请选择发送类型">
          <el-option v-for="item in options.sendType_options" :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue">
    ...
    <el-table
      ...
      <el-table-column prop="sendType" label="发送类型" align="center" v-if="columns.showColumn('sendType')">
        <template #default="scope">
          <dict-tag :options="options.sendType_options" :value="scope.row.sendType" />
        </template>
      </el-table-column>
      <el-table-column label="操作" width="80">
        <template #default="scope">
          <el-button type="danger" icon="delete" title="删除" v-hasPermi="['smscodelog:delete']" @click="handleDelete(scope.row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
...
const state = reactive({
  ...
  options: {
    ...
    sendType_options: [{ dictLabel: '登录', dictValue: '1' }]
  }
})
