<!--
 * @Descripttion: (耗品储位信息/Consumable_Storage_Space)
 * @Author: (admin)
 * @Date: (2024-05-07)
-->
<template>
  <div>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" v-hasPermi="['consumable:storageSpace:add']" plain icon="plus" @click="handleAdd">
          {{ $t('btn.add') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button type="success" v-hasPermi="['consumable:storageSpace:query']" plain icon="grid" @click="handleQRCode"> 二维码 </el-button>
      </el-col>
      ...
      <el-table-column label="操作" width="160">
        <template #default="scope">
          ...
          <el-button
            ...
            v-hasPermi="['consumable:storageSpace:add']"
            type="primary"
            size="small"
            icon="plus"
            title="增加"
          ...
          <el-button
            v-hasPermi="['consumable:storageSpace:edit']"
            type="success"
            size="small"
            icon="edit"
            title="编辑"
          ...
          <el-button
            v-hasPermi="['consumable:storageSpace:delete']"
            type="danger"
            size="small"
            icon="delete"
            title="删除"
    ...
    <el-dialog :title="title" :lock-scroll="false" v-model="openQRCode" width="70%">
      <StorageQRCode v-if="openQRCode"></StorageQRCode>
    </el-dialog>
  </div>
</template>

<script setup name="consumablestoragespace">
import {
  treeConsumableStorageSpace,
  listStorageSpace,
  addStorageSpace,
  delStorageSpace,
  updateStorageSpace,
  getStorageSpace
} from '@/api/consumable/consumableStorageSpace.js'
import StorageQRCode from './StorageQRCode.vue'
...
function getList() {
  loading.value = true
  treeConsumableStorageSpace(queryParams)
    .then((res) => {
      const { code, data } = res
      if (code == 200) {
