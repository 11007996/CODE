  ...
  <div class="app-container">
    <el-form :model="queryParams" label-position="left" inline ref="queryForm" v-show="showSearch" @submit.prevent>
      ...
      <el-form-item label="是否公开" prop="isPublic">
        <el-radio-group v-model="queryParams.isPublic" @change="handleQuery()">
          <el-radio-button value="">全部</el-radio-button>
          <el-radio-button v-for="item in options.isPublic_options" :key="item.dictValue" :value="item.dictValue">
            {{ item.dictLabel }}
          </el-radio-button>
        </el-radio-group>
      </el-form-item>
    ...
    <el-table :data="dataList" v-loading="loading" highlight-current-row @sort-change="sortChange" ref="table">
      ...
      <el-table-column label="操作" align="center" width="140" fixed="right">
        <template #default="scope">
          <el-button text icon="edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:article:update']">编辑</el-button>
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row)" style="margin-left: 10px">
            <template #reference>
              <el-button text icon="delete" v-hasPermi="['system:article:delete']">删除</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
...
/** 新增按钮操作 */
function handleAdd() {
  router.replace({ path: '/tool/article/publish' })
}
...
function handleUpdate(row) {
  router.push({ path: '/tool/article/publish', query: { cid: row.cid } })
}
// 详情
