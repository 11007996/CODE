    ...
    <el-form :model="queryParams" inline @submit.prevent ref="queryRef" v-show="searchToggle">
      <el-form-item prop="taskType">
        <el-select clearable v-model="queryParams.taskType" placeholder="请选择任务类型">
          <el-option v-for="item in options.taskType_options" :key="item.dictValue" :label="item.dictLabel" :value="parseInt(item.dictValue)" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item prop="isStart">
        <el-radio-group v-model="queryParams.isStart" @change="handleQuery()">
          ...
          <el-radio-button :value="item.dictValue" v-for="item in options.isStart_options">{{ item.dictLabel }}</el-radio-button>
        </el-radio-group>
      </el-form-item>
    ...
    <el-table v-if="viewSwitch == 1" ref="tasks" v-loading="loading" :data="dataTasks" border row-key="id" @sort-change="handleSortable">
      ...
      <el-table-column prop="taskType" label="任务类型" align="center" v-if="columns.showColumn('taskType')">
        <template #default="scope">
          <dict-tag :options="options.taskType_options" :value="scope.row.taskType" />
        </template>
      </el-table-column>
      <el-table-column prop="triggerType" label="触发器类型" align="center" v-if="columns.showColumn('triggerType')">
        <template #default="scope">
          <dict-tag :options="options.triggerType_options" :value="scope.row.triggerType" />
        </template>
      </el-table-column>
      <el-table-column sortable prop="isStart" align="center" label="任务状态" width="100">
        <template #default="scope">
          <dict-tag :value="scope.row.isStart" :options="options.isStart_options"></dict-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="jobGroup"
    ...
    <el-row :gutter="20" v-if="viewSwitch == 2">
      <el-col v-for="item in dataTasks" :lg="8" :span="24">
        <el-card :body-style="{ padding: '15px 15px 0' }">
          <el-descriptions :column="1" :title="item.name" border>
            <el-descriptions-item label="任务类型">
              <dict-tag :options="options.taskType_options" :value="item.taskType" />
            </el-descriptions-item>
            <el-descriptions-item label="触发器类型" width="90px">
              <dict-tag :options="options.triggerType_options" :value="item.triggerType" />
            </el-descriptions-item>
            <el-descriptions-item label="任务状态" width="90px">
              <dict-tag :options="options.isStart_options" :value="item.isStart"></dict-tag>
            </el-descriptions-item>
        ...
        <el-row>
          ...
          <el-col :lg="12">
            <el-form-item label="触发器类型" prop="triggerType">
              <el-select v-model="form.triggerType" placeholder="请选择触发器类型" style="width: 100%">
                <el-option
                  v-for="item in options.triggerType_options"
                  :key="item.dictValue"
          ...
          <el-col :lg="12">
            <el-form-item label="任务分组" maxlength="200" prop="jobGroup">
              <el-select v-model="form.jobGroup" placeholder="请选择任务分组">
                <el-option
                  v-for="dict in options.jobGroup_options"
                  :key="dict.dictValue"
          ...
          <el-col :lg="24">
            <el-form-item label="传入参数" prop="jobParams">
              ...
              <el-input type="textarea" v-model="form.jobParams" placeholder="传入参数" />
            </el-form-item>
          </el-col>
...
proxy.getDicts('sys_job_group').then((response) => {
  state.options.jobGroup_options = response.data
})
...
function handleJobLog(row) {
  ...
  if (dateRangeCreateTime && dateRangeCreateTime.value.length == 2) {
    logForm.beginTime = dateRangeCreateTime.value[0]
    logForm.endTime = dateRangeCreateTime.value[1]
  }
