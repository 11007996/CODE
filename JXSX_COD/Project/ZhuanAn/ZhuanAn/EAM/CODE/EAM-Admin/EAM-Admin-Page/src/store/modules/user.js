import { login, logout, getInfo, oauthCallback, phoneLogin, switchFactory } from '@/api/system/login'
...
import { encrypt, cusEncrypt } from '@/utils/jsencrypt'
...
const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    login(userInfo) {
      ...
      const password = userInfo.useOaAccount
        ? cusEncrypt(userInfo.password, userInfo.serverRsaPublicKey).toString()
        : md5(userInfo.password).toString()
      ...
      return new Promise((resolve, reject) => {
        login(username, password, code, uuid, clientId, factoryId, useOaAccount)
          .then((res) => {
            if (res.code == 200) {
              setToken(res.data)
              this.token = res.data
      ...
      return new Promise((resolve, reject) => {
        getInfo()
          .then((res) => {
            ...
            this.factoryId = data.factory.factoryId //当前登入厂区
            this.factoryName = data.factory.factoryName
            const host = window.location.host
            Cookies.set('factoryId', data.factory.factoryId, { expires: 30, path: host })
            resolve(res)
          })
          .catch((error) => {
            console.warn(error)
