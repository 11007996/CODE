<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="业务类型" prop="businessType">
        <el-select v-model="queryParams.businessType" placeholder="操作类型" clearable>
          <el-option v-for="dict in options.businessType_options" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item label="操作状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="操作状态" clearable>
          <el-option v-for="dict in options.status_options" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" border :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作id" width="60" align="center" prop="operId" :show-overflow-tooltip="true" v-if="columns.showColumn('operId')" />
      <el-table-column
        label="系统模块"
        min-width="160"
        align="center"
      ...
      <el-table-column label="业务类型" width="90" align="center" prop="businessType" v-if="columns.showColumn('businessType')">
        <template #default="scope">
          <dict-tag :options="options.businessType_options" :value="scope.row.businessType" />
        </template>
      </el-table-column>
      <el-table-column label="请求方法" width="90" align="center" prop="requestMethod" v-if="columns.showColumn('requestMethod')" />
      <el-table-column label="操作人员" width="90" align="center" prop="operName" v-if="columns.showColumn('operName')" />

      <el-table-column label="操作地址" width="160" prop="operIp">
        <template #default="{ row }">
          <div>{{ row.operLocation }}</div>
          <div>{{ row.operIp }}</div>
        </template>
      </el-table-column>
      <el-table-column label="操作状态" width="90" align="center" prop="status" v-if="columns.showColumn('status')">
        <template #default="{ row }">
          <dict-tag :options="options.status_options" :value="row.status"></dict-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="日志内容" min-width="250" prop="errorMsg" v-if="columns.showColumn('errorMsg')" />
      <el-table-column label="操作日期" width="160" align="center" prop="operTime" v-if="columns.showColumn('operTime')">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="160">
        <template #default="scope">
          <el-button text icon="view" @click="handleView(scope.row, scope.index)" v-hasPermi="['monitor:operlog:query']"> 详细 </el-button>
          <el-button text icon="delete" @click="handleDelete(scope.row)" v-hasPermi="['monitor:operlog:remove']"> 删除 </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          ...
          <el-col :lg="12">
            <el-form-item label="操作类型：">
              <dict-tag :options="options.businessType_options" :value="form.businessType" />
            </el-form-item>
          </el-col>
          <el-col :lg="12">
            <el-form-item label="操作状态：">
              <dict-tag :options="options.status_options" :value="form.status"></dict-tag>
            </el-form-item>
          </el-col>
          <el-col :lg="12">
