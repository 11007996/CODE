<template>
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :lg="18" :xm="24">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="primary" plain @click="handleSyncUserToEmp" v-hasRole="['admin']"> 同步到员工 </el-button>
          </el-col>
        ...
        <el-row :gutter="20">
          ...
          <el-col :lg="24">
            <el-form-item label="厂区">
              <el-select v-model="form.factoryIds" multiple placeholder="请选择厂区" style="width: 100%">
                <el-option
    ...
    <!-- 用户同步到员工对话框 -->
    <el-dialog title="同步用户到员工" v-model="openSync" width="400px" append-to-body>
      <el-col :lg="24">
        <el-form-item label="厂区">
          <el-select v-model="syncForm.factoryIds" multiple placeholder="请选择厂区" style="width: 100%">
            <el-option v-for="item in factoryOptions" :key="item.dictValue" :label="item.dictValue + ':' + item.dictLabel" :value="item.dictValue">
            </el-option>
          </el-select>
        </el-form-item>
      </el-col>
      <template #footer>
        <el-button @click="openSync = false">{{ $t('btn.cancel') }}</el-button>
        <el-button type="primary" @click="submitSyncForm">{{ $t('btn.submit') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { changeUserStatus, listUser, resetUserPwd, delUser, getUser, updateUser, addUser, exportUser, syncUserToEmp } from '@/api/system/user'
...
function initFactorys() {
  ...
  dictFactory(params).then((res) => {
    factoryOptions.value = res.data.result
  })
}
...
function handleSyncUserToEmp() {
  openSync.value = true
}

function submitSyncForm() {
  syncForm.value.userNames = usernames.value
  syncUserToEmp(syncForm.value).then((response) => {
