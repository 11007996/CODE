...
{
    public partial class FrmAutomationTest : Form
    {
        ...
        private bool DEVICE_ARRIVAL_FLAG = false;
        private TestStateEnum GroableTestState = TestStateEnum.Ready;

        //全局测试状态
        private SerialPortProxy SerialPortProxy = new SerialPortProxy();

        private SynchronizationContext SyncContext = null;//异步上下文
        private Dictionary<string, TestResult> TestResultDict = new Dictionary<string, TestResult>();//项目测试结果字典
                                                                                                     //串口通信代理
                                                                                                     //设备是否就绪

        public FrmAutomationTest()
        {
            InitializeComponent();
        ...
        private void FrmAutomationTest_Load(object sender, EventArgs e)
        {
            //启动模式
            ...
            //-----------------基本设置绑定------------------
            if (Setting.AutomationTest.TestItems.Count == 0)
                Setting.AutomationTest.TestItems.Add(new TestItemConfig());
            dgvTestItem.DataSource = Setting.AutomationTest.TestItems;
            cmbModel.DataBindings.Add("SelectedItem", Setting.AutomationTest, "Model");
            if (Setting.AutomationTest.SerialProt == null)
                Setting.AutomationTest.SerialProt = new SerialPortConfig();
            //串口
            txbHexSuffix.DataBindings.Add("Text", Setting.AutomationTest, "SuffixHexCode");
            txbStartTestHexCode.DataBindings.Add("Text", Setting.AutomationTest, "StartTestHexCode");
            cmbPortName.DataBindings.Add("SelectedItem", Setting.AutomationTest.SerialProt, "PortName");
            cmbBaudRate.DataBindings.Add("Text", Setting.AutomationTest.SerialProt, "BaudRate");
            tbDataBits.DataBindings.Add("Text", Setting.AutomationTest.SerialProt, "DataBits");
            cmbStopBits.DataBindings.Add("SelectedItem", Setting.AutomationTest.SerialProt, "StopBits");
            cmbParity.DataBindings.Add("SelectedItem", Setting.AutomationTest.SerialProt, "Parity");
            chkSerailListenFlag.DataBindings.Add("Checked", Setting.AutomationTest, "SerailListenFlag");
        }
        ...
        private void FrmAutomationTest_FormClosing(object sender, FormClosingEventArgs e)
        {
            StopTest();
        ...
        private void StartTest()
        {
            ...
            GroableTestState = TestStateEnum.Ready;
        ...
        private void StopTest()
        {
            ...
            GroableTestState = TestStateEnum.Complete;
        ...
        {
            if (GroableTestState == TestStateEnum.Ready)
            {
                GroableTestState = TestStateEnum.Testing;
                if (Setting.AutomationTest.Model == TestStartModelEnum.设备就绪 || Setting.AutomationTest.Model == TestStartModelEnum.上位机)
                {
                    //等待设备就绪
                    while (!DEVICE_ARRIVAL_FLAG && GroableTestState == TestStateEnum.Testing)
                    {
                        Thread.Sleep(100);
                    }
                }

                foreach (TestItemConfig item in Setting.AutomationTest.TestItems)
                {
                    if (GroableTestState == TestStateEnum.Testing)
                        SyncContext.Post(TestItemCallBack, item);
                    while (GroableTestState == TestStateEnum.Testing)
                    {
        ...
        {
            if (dgvTestItem.CurrentCell != null)
            {
                if (dgvTestItem.CurrentCell.RowIndex >= 0)
                {
                    ...
                    Setting.AutomationTest.TestItems.Remove(config);
                }
                else
                {
        ...
        private void InitItemResultLayout()
        {
            ...
            foreach (var item in Setting.AutomationTest.TestItems)
            {
                // panelResult
        ...
        private void dgvTestItem_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                int idx = Setting.AutomationTest.TestItems.IndexOf(testItem);
                //项目配置
                if (dgvTestItem.Columns[e.ColumnIndex].Name == "dgcTestConfig")
                {
                    FrmTestItemConfig frm = new FrmTestItemConfig(testItem);
                    frm.ShowDialog();
                    if (frm.TestItemConfig != null)
                    {
                        Setting.AutomationTest.TestItems[idx] = frm.TestItemConfig;
                        dgvTestItem.DataSource = null;
                        dgvTestItem.DataSource = Setting.AutomationTest.TestItems;
                    }
                }
        ...
        private void InitSerailPort()
        {
            ...
            {
                SerialPortProxy.Open(Setting.AutomationTest.SerialProt, this.HandleMsg);
            }
            else
            {
                SerialPortProxy.Close();
        ...
        private void bgWorkTest_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            {
                AddLog($"{item.TestItem}测试结果:{TestResultDict[item.TestItem].State}");
            }
        }
        ...
        public void ShowTestResult(TestResult result)
        {
            ...
            {
                if (result.TestItem.Equals(panel.Tag))
                {
        ...
        {
            //结束测试，发送给PLC
            if (Setting.AutomationTest.SerailListenFlag)
            {
        ...
        private void HandleMsgCallBack(object state)
        {
            ...
            AddLog($"接收指令:{hexStr}");
            //取前面2字节，表示请求
            if (hexStr.EndsWith(Setting.AutomationTest.SuffixHexCode))
            {
                //int prefixSize = string.IsNullOrWhiteSpace(BaseInfo.PrefixHexCode) ? 0 : BaseInfo.PrefixHexCode.Split('-').Length;//前缀字节个数
                int suffixSize = string.IsNullOrWhiteSpace(Setting.AutomationTest.SuffixHexCode) ? 0 : Setting.AutomationTest.SuffixHexCode.Split('-').Length;//后缀字节个数
                ...
                //请求代码
                if (codes[0] == Setting.AutomationTest.StartTestHexCode)
                { //请求开始测试
        ...
        {
            if (item == null) return;
            if (GroableTestState == TestStateEnum.Complete)
            {
                if (Setting.AutomationTest.SerailListenFlag)
                {
                    string hexText = "";
                    ...
                    if (!string.IsNullOrEmpty(Setting.AutomationTest.SuffixHexCode))
                        hexText += ("-" + Setting.AutomationTest.SuffixHexCode);
